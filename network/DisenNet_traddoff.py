...
class DisenNet(nn.Module):
    def __init__(self, c_id=256):
        super(DisenNet, self).__init__()
        self.encoder = MLAttrEncoder()
        self.generator = AADGenerator(c_id)

    def forward(self, Xt, z_id,Xs,la):
        attr = self.encoder(Xt)
        attr_s = self.encoder(Xs)

        Y = self.generator([la*attr[i]+(1-la)*attr_s[i]  for  i in range(len(attr))], z_id)
        return Y, attr
